...
def proc_image_cam2(img_cm2, unique_id) :
    ...
    img_cm2.save_to_disk('_out_images/'+ unique_id + '_rt' + '.png')
...
def main():
    ...
    try:
        ...
        rgb_camera_transform_2 = carla.Transform(carla.Location(x=0.8, y=R_CAMERA_OFFSET, z=1.7))
            ...
            while True:
                ...
                if((current_frame - start_frame) > 50) :
                    labels = {
                        ...
                        'relative_angle' : relative_angle_from_centre, \
                    ...
                    labels_r = {
                        ...
                        'front_vehicle' : front_vehicle_distance, \
                        'pedestrian_distance' : pedestrian_distance, \
                        'centre_dist'   : distance_of_the_waypoint_r, \
                        'relative_angle' : relative_angle_from_centre, \
                        'traffic_light' : traffic_light_state \
                        }
                    print(labels_r['front_vehicle'])
                    str_timestamp              = str(timestamp)
                    label_dict[str_timestamp] = labels
                    label_dict_r[str_timestamp] = labels_r
                    # Draw the display.
        ...
        with open('_out/test_label_out_r.pickle', 'wb') as handle:
            pickle.dump(label_dict_r, handle) #, protocol=pickle.HIGHEST_PROTOCOL)
        print("Reading pickle....")
        with open('_out/test_label_out.pickle', 'rb') as handle:
            label_dict_rd = pickle.load(handle)
        with open('_out/test_label_out_r.pickle', 'rb') as handle:
            label_dict_rd = pickle.load(handle)
